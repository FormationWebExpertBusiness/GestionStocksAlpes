...
test('test Rack method itemOn', function () {
    // datas
    $rack = Rack::create(['nb_level' => 5]);
    ...
    // test
    $this->assertEquals([1,2], $rack->itemOn()->pluck('id')->toArray());
});

test('test Rack method itemOnLevel', function () {
    // datas
    $rack = Rack::create(['nb_level' => 5]);
    ...
    // test
    $this->assertEquals([2], $rack->itemOnLevel(1)->pluck('id')->toArray());
});
