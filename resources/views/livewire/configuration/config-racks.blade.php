            ...
            <div class="sm:flex-auto">
                <div class="min-w-0 flex-1">
                    <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">
                        Configuration des Étagères</h2>
                </div>
                <p class="mt-2 text-sm text-gray-700">Ajouter, modifier ou supprimer des étagères</p>
            </div>
                                    ...
                                    @forelse ($racks as $rack)
                                        <div wire:key="rack-{{ $rack->id }}">
                                            <tr class="odd:bg-white even:bg-gray-50 divide-x divide-gray-200 table w-full table-fixed">
                                                ...
                                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 w-1/4">
                                                    <div class="inline-block px-3">
                                                        @livewire('details.rack.detail-modal', ['rack' => $rack], key('rack-detail-'.$rack->id))
                                                    </div>
                                                    <div class="inline-block px-3">
                                                        @livewire('forms.rack.rack-edit-form', ['rack' => $rack] , key('rack-delete-'.$rack->id))
                                                    </div>
                                                    <div class="inline-block px-3">
                                                        <button wire:click="openWarningDelete({{ $rack->id }})" class="inline-flex items-center rounded-md border border-transparent bg-white p-3 text-red-600 shadow hover:bg-red-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
